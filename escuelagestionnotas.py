...
def crear():
  ...
  ##  miConexion=sqlite3.connect("base")
    ##miCursor=miConexion.cursor()
    try:
        t=((float(notaPrimer.get())+float(notaSegunda.get())+float(notaTercera.get()))/3)
        prom=float(t)
        datos=dni.get(),Nombre.get(),Curso.get(),notaPrimer.get(), notaSegunda.get(), notaTercera.get(),prom
...
def actualizar():
    ...
    try:
        ...
        prom=float(t)
        datos=dni.get(),Nombre.get(),Curso.get(),notaPrimer.get(),notaSegunda.get(),notaTercera.get(),prom
        miCursor.execute("UPDATE alumno SET dni=?, NOMBRE=?,CURSO=?,NOTAPRIMER=?, NOTASEGUNDA=?, NOTATERCERA=?,PROMEDIO=? WHERE dni="+dni.get(),(datos))
        miConexion.commit()
    except:
...
def borrar():
     ...
     try:
         if messagebox.askyesno(message="¿Realmente quieres eliminar el registro?", title="ADVERTENCIA"):
            miCursor.execute("DELETE FROM alumno WHERE dni="+dni.get())
            miConexion.commit()
     except:
